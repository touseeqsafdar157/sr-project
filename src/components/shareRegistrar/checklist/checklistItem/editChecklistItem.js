...
import { updateNewChecklistItem } from "../../../../store/services/checklist.service";
...
export default function EditChecklistItem() {
  ...
  const handleUpdateChecklistItem = async () => {
    if (item_id == "") {
      setItem_idError(true);
    } else {
      setItem_idError(false);
    }
    if (checklist_id == "") {
    ...
    if (
      ...
      try {
        setLoading(true);
        const response = await updateNewChecklistItem(
          email,
          item_id,
          checklist_id,
          txn_type,
                ...
                <div className="mb-3">
                  ...
                  {checklist_options.length > 0 ? (
                    <Select
                      ...
                      defaultInputValue={() => {
                        return checklist_options.find(
                          (item) => item.value == checklist_id
                        ).label;
                      }}
                      styles={darkStyle}
                    />
        ...
        <div className="row">
          <div className="col-md-12">
            <LoadableButton
              loading={loading}
              title="Update"
              methodToExecute={handleUpdateChecklistItem}
            />
          </div>
        </div>
      </Fragment>

...
import { updateProcessChecklist } from "../../../../store/services/checklist.service";
...
export default function EditChecklist() {
  const data =
    JSON.parse(sessionStorage.getItem("selectedProcessChecklist")) || "";
  ...
  useEffect(async () => {
    ...
    return () => {
      sessionStorage.setItem("selectedProcessChecklist", JSON.stringify({}));
    };
  }, []);
  ...
  const handleUpdateChecklist = async () => {
    ...
    if (checklist_id !== "" && txn_type !== "" && check_list_title !== "") {
      const email = sessionStorage.getItem("email");
      try {
        setLoading(true);
        const response = await updateProcessChecklist(
          email,
          checklist_id,
          txn_type,
          check_list_title,
        ...
        <div className="row">
          <div className="col-md-12">
            <LoadableButton
              loading={loading}
              title="Update"
              methodToExecute={handleUpdateChecklist}
            />
          </div>
        </div>
      </Fragment>

...
import { updateRequirment  } from 'store/services/company.service';
import { getAllRequirmentData } from 'store/services/company.service';
export const EditStatuarRequirment = ({setEditRequirment, getPaginatedRequirment}) => {
    const requirment = JSON.parse(sessionStorage.getItem("selectedrequirment")) || "";
    const [defaultDefendendent, setDefaultDependent] = useState(requirment?.dependent_on || '')
    ...
    useEffect(()=>{
      ...
      const handleAlertRequirment = async(data)=>{
        ...
        try {
          setLoading(true);
          // let response;
          const response = await updateRequirment(
            baseEmail,
           data?.form_code,
           data?.company_type,
           data?.title,
           ...
           data?.frequency,
           data?.level_ddl,
           defaultDefendendent || selectedDependendent || '',
           data?.days_dependent,
           data?.notify_days,
           data?.notify_via,
           data?.active
...
<select
              ...
              <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        placeholder={defaultDefendendent || 'Enter Dependent '}
                        onChange={(selected) => {
                          if(selected?.label){
    ...
    {/* <form onSubmit={handleSubmit(handleAlertMessage)}>
      ...
      <div className="card ">
          <div className="card-header b-t-primary">
           <h5> Edit Statuary Requirment</h5>
            </div>
            <div className="row">

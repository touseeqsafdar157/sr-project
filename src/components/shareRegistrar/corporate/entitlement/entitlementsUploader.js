...
import { uploadEntitlement } from "store/services/corporate.service";
...
export default function EntitlementUploader() {
  // User Email
  ...
  const {
    ...
    control,
    setValue,
  } = useForm({ resolver: yupResolver(entitlementUploaderSchema) });
  ...
  const handleUploadEntitlement = async (data) => {
    setLoading(true);
    try {
      const response = await uploadEntitlement(
        email,
  ...
  return (
    <Fragment>
      ...
      <h6 className="text-nowrap mt-3 ml-3">Electronic Entitlement Uplaoder</h6>
      <Breadcrumb title="Electronic Entitlement Uploader" parent="Corporate" />
      </div>

      <form onSubmit={handleSubmit(handleUploadEntitlement)}>
        <div className="row">
          <div className="col-sm-12 col-md-12">
            <div className="card ">
              <div className="card-header b-t-primary">
                {/* <h3>Electronic Entitlement Uploader</h3> */}
              </div>
              <div className="card-body">
                <div className="row">
                  ...
                  <div className="col-sm-12 col-md-3 col-lg-3">
                    <div className="form-group my-2">
                      <label htmlFor="as_per_date">As per Date</label>
                      <input
                        className={`form-control ${
                          errors.as_per_date && "border border-danger"
                        }`}
                        name="as_per_date"
                        type="date"
                        {...register("as_per_date")}
                        defaultValue={getvalidDateYMD(new Date())}
                      />
                      <small className="text-danger">
                        {errors.as_per_date?.message}
                      </small>
                    </div>
                  </div>
                </div>

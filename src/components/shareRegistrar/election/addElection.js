...
export default function AddElection({ setViewAddPage }) {
  ...
  const handleAddElection = async (data) => {
    ...
    try {
      setLoading(true);
      const response = await addElection(
        email,
        ...
        data.application_to,
        data.candidates.map((item) => ({ investor_id: item.investor_id.value }))
      );
  ...
  return (
    <div>
      <Fragment>
        <form onSubmit={handleSubmit(handleAddElection)}>
          <div className="row">

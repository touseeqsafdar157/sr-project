...
export default function ShareholdingHistory() {
    ...
    const getAvailableDates = async () => {
      ...
      try {
        ...
        !!error?.response?.data?.message
          ...
          : console.log("CDC Dates Not Found");
  ...
  const displayShareholdersPerPage = serachedShareholders
    ...
    .map((shareholder, i) => (
      <tr key={i}>
        <td>{shareholder.folio_number.split("-")[1]}</td>
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          const mapped_shareholders = cdc_shareholders
            ...
            .map((holder, i) => {
              let currentHolder;
              const current_shareholder = shareholders_data.filter(
                (item) => {
                  if (holder.folio_number.replace(`${selectedCompany}-`, "") !=='' && (holder.folio_number.replace(`${selectedCompany}-`, "") === item.folio_number.replace(`${selectedCompany}-`, ""))) {
                    currentHolder = item
                    console.log('folio_number',item)
                  } else if (holder?.shareholder_id !== '' && (holder?.shareholder_id === item?.shareholder_id)) {
                    currentHolder = item
                    console.log('cnic', holder?.shareholder_id, ' === ', item?.shareholder_id)
                  } else if (holder?.shareholder_id !== '' && (holder?.shareholder_id === item?.cnic)) {
                    currentHolder = item
                    console.log('cnic', holder?.shareholder_id, ' === ', item?.cnic)
                  } else if (item?.ntn !=='' && (holder?.shareholder_id === item?.ntn)) {
                    currentHolder = item
                    console.log('ntn',item)
                  }
              ...
              if (keys != -1) {
                ...
                return { ...currentHolder, ...holder, ...{ city: current_shareholder?.city || '' }, ...{ city: current_shareholder?.zakat_status || '' } };
              }
    ...
    return {
      ...
      folio_number:
        shareholder.cdc_key === "NO"
          ? shareholder.folio_number.split("-")[1]
  ...
  const generateDataForPhysicalReport = (shareholder) => {
    ...
    if (shareholder.folio_number.split("-")[1] == "0") {
      return {
        ...joint_one_holder,
        ...joint_two_holder,
        ...joint_three_holder,
        ...
        passport_expiry: passport_expiry || ' ',
        "father/husband_name":
          shareholder.folio_number.split("-")[1] == "0"
            ? " "
        ...
        ..._.pick(
          {
            ...
            folio_number: data.folio_number.split("-")[1],
            percentage:
              ((parseInt(data.electronic_shares) ||
                parseInt(data.physical_shares)) /
                _.sum(

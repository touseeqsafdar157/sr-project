...
export default function InvestorRequestReport() {
  ...
  const [selectedInvestorRequest, setSelectedInvestorRequest] = useState("TOS");
  ...
  const [selectedTransactionType, setSelectedTransactionType] = useState("TOS");
  ...
  const transactionTypeDropdown = [
    ...
    { label: "Transafer of Shares", value: "TOS" },
                  ...
                  <i
                    ...
                    onClick={() => {
                      ...
                      const selected_from_shareholder = shareholders.find(
                        (item) => {
                          return item.folio_number === obj?.folio_number;
                        }
                      );
                      const selected_to_shareholder = shareholders.find(
                        (item) => item.folio_number === obj?.to_folio_number
                      );
                      const selected_from_investor = investors.find((item) => {
                        return (
                          item.investor_id ===
                          selected_from_shareholder?.shareholder_id
                        );
                      });
  ...
  return (
    <Fragment>
      <div className="d-flex justify-content-between">
        <h6 className="text-nowrap mt-3 ml-3">Transfer of Share Report</h6>
        <Breadcrumb title="Transfer of Share Report" parent="Reporting" />
      </div>
          ...
          <div className="col-sm-12">
            <div className="card">
              ...
              transaction_requests.length == 0 &&
                ...
                {(investor_request_loading === true) && <Spinner />}
              { companies.length !== 0 &&
                // shareholders.length !== 0 &&
                ...
                // searchedInvestorsRequests.length !== 0 &&
                selectedTransactionType == "TOS" &&
                transaction_requests && transaction_requests.length > 0 ? (
                  <>
                    <PDFExport
                      paperSize="A4"
                      margin="1.5cm"
                          ...
                          <>
                            <div className="table-responsive">
                              ...
                              >
                                ...
                                <tbody>
                                  {transaction_requests && transaction_requests.length > 0 && transaction_requests.map((item, index) => {
                                    return (
                                      <tr key={index}>
                                        <td>
                                        ...
                                        <td className="text-right">
                                          ...
                                          { 
                                          IsJsonString(item?.input_certificates) ?
                                        ...
                                        <td className="text-right">
                                          {
                                            IsJsonString(item?.input_certificates) && IsJsonString(JSON.parse(item.input_certificates)[0].distinctive_no) ?
                                            JSON?.parse(
                                              JSON?.parse(
                                                item?.input_certificates
                                              )[0]?.distinctive_no
                                        ...
                                        <td className="text-right">
                                          ...
                                          {
                                            IsJsonString(item?.input_certificates) && IsJsonString(JSON.parse(item.input_certificates)[JSON.parse(item.input_certificates).length - 1].distinctive_no) ?
                                           JSON?.parse(
                                              JSON?.parse(
                                                item?.input_certificates
                                              )[

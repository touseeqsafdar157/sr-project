...
import { updateShare } from "../../../store/services/shareholder.service";
import LoadableButton from "../../common/loadables";
import Select from "react-select";
import { LoadableInput } from "../../common/loadables";
...
export default function EditShare() {
  const data = JSON.parse(sessionStorage.getItem("selectedShare")) || "";
  useEffect(async () => {
    try {
    ...
    return () => {
      sessionStorage.setItem("selectedShare", JSON.stringify({}));
    };
  }, []);

  const [offer_id, setOffer_id] = useState(data.offer_id);
  const [symbol, setSymbol] = useState(data.symbol);
  const [type, setType] = useState(data.type);
  const [offer_volume, setOffer_volume] = useState(data.offer_volume);
  const [offer_price, setOffer_price] = useState(data.offer_price);
  const [final_offer_price, setFinal_offer_price] = useState(
    data.final_offer_price
  );
  const [bb_percent, setBb_percent] = useState(data.bb_percent);
  const [bb_from, setBb_from] = useState(data.from);
  const [bb_to, setBb_to] = useState(data.bb_to);
  const [strike_price, setStrike_price] = useState(data.strike_price);
  const [bid_volume, setBid_volume] = useState(data.bid_volume);
  const [ipo_percent, setIpo_percent] = useState(data.ipo_percent);
  const [ipo_from, setIpo_from] = useState(data.ipo_from);
  const [ipo_to, setIpo_to] = useState(data.ipo_to);
  const [subscribed_volume, setSubscribed_volume] = useState(
    data.subscribed_volume
  );
        ...
        <div className="row">
          <div className="col-md-12">
            <LoadableButton
              loading={loading}
              title="Update"
              methodToExecute={handleUpdateShare}
            />
          </div>
        </div>
      </Fragment>

...
const TransmissionOfSharesItem = ({
  certificates,
  folios,
  num,
  startCalculation,
  ...
  const [selectedFolio, setSelectedFolio] = useState(df_folio || "");
  useEffect(() => {
    if (calculated === true) {
      startCalculation({
  ...
  const associatedCertifcate = () => {
    const selectedCert =
      ...
      IsJsonString(
        ...
        ? JSON.parse(
            certificates.find(
              (cert) => selectedCertifcate === cert.certificate_no
            )?.distinctive_no
          )
        : certificates.find(
            (cert) => selectedCertifcate === cert.certificate_no
      ...
      <tr>
        ...
        <td>
          <input
            ...
            value={numberWithCommas(associatedCertifcate().shares_count || df_count)}
            readOnly
          />
        </td>
        <td>
        ...
        <td>
          <Select
            isLoading={folios && folios.length === 0}
            options={folios && folios}
            defaultValue={
              df_folio ? { value: df_folio, label: df_folio } : null
            }
            isDisabled={df_folio}
            id="trans_folio_number"
...
export default TransmissionOfSharesItem;

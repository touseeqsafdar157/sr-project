...
export default function EditShareholder({ setViewEditPage }) {
  ...
  const checkIndividuals = () =>
    holder?.category?.toUpperCase() === "INDIVIDUALS";
  const checkDirectors = () => holder?.category?.toUpperCase() === "DIRECTORS";
  const checkExecutives = () => holder?.category?.toUpperCase() === "EXECUTIVES";
  const checkEmployee = () => holder?.category?.toUpperCase() === "EMPLOYEE";
  const checkSponser = () =>
  holder?.category?.toUpperCase() === "SPONSORS";

  const [type, setType] = useState(
    holder?.category == undefined ? "" : holder?.category?.toUpperCase()
  );
  ...
  const [sponserNTN, setSponserNTN] = useState(holder?.cnic ? 'CNIC' : 'NTN')
  // Toggle States
  ...
  useEffect(async () => {
    try {
      ...
      setShareholderName(holder.shareholder_name);
    } catch (err) {

...
export default function EditUBO({setViewEditPage}) {
  const allUBO = JSON.parse(sessionStorage.getItem("singleUBO"));
    ...
    const {
      ...
      let calculate = ((no_of_shares / total_shares) * 100).toFixed(3);
      if(+calculate===0){
        calculate = ((no_of_shares / total_shares) * 100).toFixed(0);
      }
      if(total_shares=='0') {
        calculate="0";
      }
        ...
        <div className="row">
          ...
          >
            ...
            {(category === 'INDIVIDUALS' ||
             ...
             category === 'JOINT STOCK COMPANIES' || 
             category === 'MUTITAL FUND/TRUSTEE' || 
             category === 'INSURANCE COMPANIES' || 
             category === 'INVESTMENT COMPANIES' || 
             category === 'ASSOCIATED COMPANIES' || 
             category === 'LEASING COMPANIES' || 
             category === 'TRUSTS' ||
             category === 'NIT AND ICP' || 
             category === 'MODARBA' || 
             category === 'MODARBA MANAGEMENT' || 
             category === 'CORPORATE ORGANIZATIONS' ||
             category === 'CHARITABLE INSTITUTES' || 
             category === 'CDC' ||

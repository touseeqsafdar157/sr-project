...
import { addNewTransactionType } from "../../../../store/services/transaction.service";
...
export default function AddTransactionType() {
  ...
  const handleAddTransactionType = async () => {
    ...
    if (txn_type !== "" && transaction_type !== "") {
      ...
      try {
        setLoading(true);
        const response = await addNewTransactionType(
          email,
        ...
        <div className="row">
          <div className="col-md-12">
            <LoadableButton
              loading={loading}
              title="Submit"
              methodToExecute={handleAddTransactionType}
            />
          </div>
        </div>
      </Fragment>
